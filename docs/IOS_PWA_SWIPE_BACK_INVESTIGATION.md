...
## 2026-09-17 경계 구간 실제 수정

일곱 번째 기록에서 화면 폭 390 CSS px 기준 왼쪽 21px·22px에서 시작한 오른쪽 스와이프가 outside_edge → history_pop → route_pop으로 이어졌다. 12·16·18px 등 기존 범위에서는 Flutter 제스처 복귀가 기록됐다. 기록된 150개 구간에 한정된 관찰이며 회색 화면의 정확한 표시 시점은 기록하지 않는다.

수정: 설치형 iOS PWA에서 JS 차단 폭을 32px로 확장하고 공통 상세 라우트의 Cupertino 제스처 인식 폭도 최소 32px로 조정한다. 32px는 관찰된 실패 지점에 여유를 둔 값이며 WebKit의 전체 시스템 제스처 범위를 확정한 값은 아니다.

Flutter 3.41의 Cupertino 감지기는 max(왼쪽 padding, 20)으로 폭을 정한다. PWA 전용 라우트는 전환 하위의 감지기에만 최소 32px padding을 전달하고 본문에는 원래 MediaQuery를 복원한다. 기존 애니메이션·취소·popGestureEnabled 조건을 재사용한다. 본문 여백은 유지한다. Flutter SDK 업데이트 시 이 내부 동작과 테스트를 재검증해야 한다.
