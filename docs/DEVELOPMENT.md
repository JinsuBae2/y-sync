...
* 각 기록은 **Who, When, Where, Status**를 요약 표로 작성하고, 본문은 **작업 개요(What·Why), 구현(How), 검증 및 추적(Verification·Tracking), 후속 작업(Risks / Follow-up)**으로 묶어 작성합니다.
* **Who에는 요청·검토자 등 실제 사람 이름만 기록하고 AI·에이전트·도구 이름은 기록하지 않습니다.**
...
로컬에서 테스트 계정(관리자·학생)이 필요하면 `YSYNC_SEED_PASSWORD` 환경 변수를 설정하고 백엔드를 실행합니다. 값이 없으면 경고만 남기고 시드 생성을 건너뜁니다. 예전처럼 소스에 적힌 고정 비밀번호는 없습니다.

```bash
export YSYNC_SEED_PASSWORD='원하는-비밀번호'
```

운영 프로필에서는 시드 생성 자체가 실행되지 않습니다.
