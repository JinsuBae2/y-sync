...
| **Status** | 구현 및 로컬 검증 완료, `main` 대상 PR 준비 |
...
- 실제 배포 화면에서 모바일 글꼴 크기와 관리자 의견이 많은 경우의 밀도를 최종 확인합니다.
...
- 배포 시 백엔드를 먼저 반영해 `feedback`, `feedback_image` 테이블 생성 후 프론트엔드를 반영합니다.
...
| **Status** | 구현 및 로컬 검증 완료, `main` 대상 PR #86 게시 |
...
- 운영 배포와 PR 병합은 이번 작업에 포함하지 않습니다.
...
| **Status** | 구현·검증 완료, `main` 대상 PR #83 게시 |
