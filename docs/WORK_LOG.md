...
## 2026-09-23 - 공지 작성자가 자기 공지 알림을 받던 문제

- 누가: 백엔드
- 무엇을: 공지 알림 수신자에서 작성자를 제외했습니다.
- 왜:
  - `NoticeRecipientSelector`가 활성 회원 전체를 수신자로 돌려주고 작성자를 빼지 않아, 공지를 쓴 관리자가 앱 알림함 적재와 푸시를 자기 공지로 받고 있었습니다. 공지를 연달아 올리는 학기 초에는 알림함이 자기 글로 찹니다.
  - **댓글 알림은 이미 같은 규칙을 지키고 있었습니다**(`CommentEventListener`의 "본인 글에 본인이 남긴 댓글" 스킵). 한쪽에만 빠져 있던 상태라 의도한 차이로 보기 어려웠습니다.
- 어떻게:
  - `NoticeCreatedEvent`에 `authorId`를 실었습니다. 엔티티에서 꺼내지 않은 이유는 이 이벤트가 `@Async` + `AFTER_COMMIT`에서 처리돼 그 시점의 `Notice`가 준영속이기 때문입니다. `notice.getAuthor().getId()`는 지연 로딩 프록시에서도 식별자만 꺼내므로 지금은 동작하지만, 나중에 `getName()` 같은 걸 덧붙이면 조용히 깨집니다. `CommentCreatedEvent`가 이미 쓰던 방식입니다.
  - 작성자 제외는 **학년 필터보다 먼저** 겁니다. 필터를 켜든 끄든 항상 적용돼야 합니다.
  - `authorId`가 null이면 아무도 제외하지 않습니다.
- 언제·어디서: 2026-09-23, `fix/notice-author-self-notification` 브랜치.
- 검증: 백엔드 테스트 전체 통과. 제외 필터를 일부러 빼고 추가한 테스트 2건이 실제로 실패하는 것을 확인한 뒤 원복했습니다. 학년 필터가 꺼진 경우에도 제외되는지, 작성자를 모를 때는 아무도 빠지지 않는지도 함께 고정했습니다.
