...
#### 12. 가입 전 학생 정보 확인 (`POST /auth/verify-student`)
- 실패 응답은 계정 상태를 구분하지 않습니다. 등록되지 않은 학번과 이름 불일치는 같은 문구를 반환합니다.
- 이미 가입이 완료된 경우만 구분해 로그인을 안내합니다.
- `GET /auth/check-duplicate`는 제거되었습니다. 비인증으로 학번 등록 여부를 반환하던 엔드포인트이며 사용하는 화면이 없었습니다.

#### 13. 가입 인증 증표 (`POST /auth/verify-student/verify-code` → `POST /auth/signup`)
- 인증 코드 검증에 성공하면 응답에 `verificationGrant`가 포함됩니다. 인증을 통과한 요청에만 반환됩니다.
- `POST /auth/signup`은 이 값을 `verificationGrant` 필드로 함께 받아야 하며, 없거나 일치하지 않으면 가입을 거부합니다.
- 증표는 한 번 사용하면 즉시 폐기되어 재사용할 수 없습니다. 같은 증표로 동시에 들어온 요청 중 하나만 성공합니다.
- 유효 시간은 인증 통과 후 10분입니다.
- 클라이언트는 증표를 저장소에 남기지 않고 가입 화면에서만 보유합니다.

#### 14. 공지 알림 수신 학년 (`PUT /members/me/notice-grade`, `PUT /admin/members/{id}`)
...
#### 15. 공지 알림 학년 선택 현황 (`GET /admin/members/notice-grade-stats`)
- **ADMIN / SUPER_ADMIN 전용**
