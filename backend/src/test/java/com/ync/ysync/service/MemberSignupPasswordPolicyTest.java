...
class MemberSignupPasswordPolicyTest {
    ...
    void 정책에_미달하는_비밀번호로는_가입할_수_없다() {
        assertThatThrownBy(() -> memberService.signup("2305001", "1", "학생", null))
    ...
    void 영문_숫자_특수문자를_모두_포함하지_않으면_거부된다() {
        assertThatThrownBy(() -> memberService.signup("2305001", "password1234", "학생", null))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("비밀번호는");

        assertThatThrownBy(() -> memberService.signup("2305001", "Password!!!!", "학생", null))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("비밀번호는");
    }
    ...
    void 정책을_만족하는_비밀번호는_정책_검사를_통과하고_인증_단계로_넘어간다() {
        assertThatThrownBy(() -> memberService.signup("2305001", "Strong1234!", "학생", null))
                .isInstanceOf(IllegalArgumentException.class)
