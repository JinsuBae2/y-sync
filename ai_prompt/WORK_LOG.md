...
- **누가(Who)**: 배진수(초기 백엔드 구현·완성 요청), Codex(백엔드 보완·Flutter 구현·검증)
- **언제(When)**: 2026-08-27, Asia/Seoul
- **어디서(Where)**: `fix/comment-soft-delete` 브랜치, Flutter 댓글 모델·댓글 UI와 Spring 댓글 도메인·서비스
- **무엇을(What)**: 작성자 댓글 삭제를 원문 보존형 소프트 삭제로 전환하고 일반 응답에서 원문·작성자 정보를 마스킹했습니다. Flutter는 `deletedBy`를 파싱해 작성자 삭제와 관리자 삭제 안내 문구·색상을 구분하며 구형 응답도 호환합니다.
- **왜(Why)**: 소프트 삭제된 댓글의 삭제 주체를 사용자 화면에서 구분하고, 사용자가 직접 작성한 백엔드 코드의 논리 오류를 배포 전에 찾기 위해서입니다.
- **어떻게(How)**: 댓글 엔티티는 삭제 상태·주체·사유만 변경하고 댓글 원문과 관계를 유지합니다. 서비스는 최초 삭제에서만 댓글 수를 감소시키며 관리자 삭제는 사유가 필요한 전용 API로 제한했습니다. 일반 DTO는 삭제된 원문과 작성자를 마스킹하고 Flutter UI는 삭제 주체별 문구를 표시합니다.
- **검증(Verification)**: 백엔드 `./gradlew compileJava` 성공, 변경한 Dart 파일 2개의 포맷 확인 및 정적 분석 통과, `git diff --check` 확인
- **추적(Tracking)**: 브랜치 `fix/comment-soft-delete`, 코드 커밋 `89a4cf2`, PR 생성 전
- **상태(Status)**: 로컬 구현·최소 검증·코드 커밋 완료
- **위험 및 후속 작업(Risks/Follow-up)**: 운영은 Hibernate `ddl-auto=update`로 nullable `deleted_by` 컬럼을 생성합니다. 배포 전 작성자 삭제·관리자 삭제·중복 삭제·관리자 복구 흐름을 확인해야 합니다.

---

## 2026-08-26 - 댓글 소프트 삭제 구현 골격 작성

- **누가(Who)**: 배진수(직접 구현 방식 요청), Codex(현황 분석·TODO 골격 작성)
- **언제(When)**: 2026-08-26, Asia/Seoul
- **어디서(Where)**: `fix/comment-soft-delete` 브랜치, 댓글 도메인·서비스·응답 DTO·Flutter 댓글 UI
- **무엇을(What)**: 작성자 댓글 삭제를 증거 보존형 소프트 삭제로 전환하기 위한 구현 위치와 요구 조건을 한국어 `TODO` 주석으로 표시했습니다.
- **왜(Why)**: 현재 관리자 삭제는 소프트 삭제지만 작성자 본인 삭제는 DB 물리 삭제이며, 이후 코드는 사용자가 직접 구현하기로 했기 때문입니다.
- **어떻게(How)**: 기존 동작은 변경하지 않고 상태 변경, 중복 댓글 수 감소 방지, 공개 응답 원문 마스킹, 삭제 주체별 UI 문구 위치만 지정했습니다.
- **검증(Verification)**: 코드 동작 변경이 없는 주석 골격이므로 테스트는 생략하고 `git diff --check`만 확인했습니다.
- **추적(Tracking)**: 브랜치 `fix/comment-soft-delete`, 커밋·PR 전
- **상태(Status)**: 구현 골격 작성 완료, 실제 소프트 삭제 미구현
- **위험 및 후속 작업(Risks/Follow-up)**: TODO 구현 전까지 작성자 삭제는 기존처럼 물리 삭제됩니다. 공개 API에서 삭제 댓글 원문이 노출되지 않도록 응답 마스킹을 반드시 함께 구현해야 합니다.

---

## 2026-08-26 - 공지·커뮤니티 조회수 집계 경로 정합성 개선

- **누가(Who)**: 배진수(기능 점검·개선 요청), Codex(경로 분석·구현·검증)
- **언제(When)**: 2026-08-26, Asia/Seoul
- **어디서(Where)**: `fix/content-filter-flicker` 브랜치, Spring Boot 공지·커뮤니티 서비스와 Flutter 목록·홈·내 글 화면
- **무엇을(What)**: 공지사항과 커뮤니티 게시글의 상세 화면 진입 시 조회수가 증가하도록 주요 진입 경로를 상세 조회 API로 통일하고, 수정·삭제 처리에서는 조회수가 증가하지 않도록 분리했습니다.
- **왜(Why)**: 목록·홈·내 글에서 이미 받은 게시글 객체를 상세 화면에 바로 전달해 실제 열람에도 조회수가 증가하지 않았고, 반대로 일부 수정·삭제 작업은 상세 조회 메서드를 재사용해 조회수가 증가했기 때문입니다.
- **어떻게(How)**: 사용자 상세 진입은 기존 상세 로딩 화면을 거쳐 GET 상세 API를 한 번 호출하고, 백엔드 수정·삭제는 조회수 증가가 없는 저장소 조회 메서드를 사용하도록 변경했습니다.
- **검증(Verification)**: 백엔드 `./gradlew compileJava` 성공, 변경 Flutter 화면 대상 `flutter analyze` 오류·경고 없음, `git diff --check` 통과
- **추적(Tracking)**: 브랜치 `fix/content-filter-flicker`, 코드 커밋 `28f7d41`, PR #26
- **상태(Status)**: 로컬 구현·최소 검증·코드 커밋 완료
- **위험 및 후속 작업(Risks/Follow-up)**: 현재 정책은 같은 사용자가 같은 글을 다시 열어도 매번 1회 증가합니다. 사용자·일자별 중복 제거가 필요하면 별도 저장 구조와 정책 설계가 필요합니다.

---

## 2026-08-26 - 커뮤니티 자기 글 수정 및 macOS Java 환경 점검

- **누가(Who)**: 배진수(기능 추가·환경 점검 요청), Codex(구현·댓글 정책 검토·환경 진단)
- **언제(When)**: 2026-08-26, Asia/Seoul
- **어디서(Where)**: `fix/content-filter-flicker` 브랜치, 커뮤니티 API·Flutter 작성/상세 화면·Android Gradle 설정·macOS 셸 환경
...
- **누가(Who)**: 배진수(운영 화면 증상 제보), Codex(원인 확인·수정)
- **언제(When)**: 2026-08-26, Asia/Seoul
- **어디서(Where)**: `fix/content-filter-flicker` 브랜치, Flutter 공용 콘텐츠 필터 위젯
- **무엇을(What)**: 필터 선택 변경 시 이전 항목과 새 항목이 동시에 깜빡이는 시각 현상을 제거했습니다.
- **왜(Why)**: 선택 상태가 바뀔 때 두 항목의 배경 전환 애니메이션과 터치 리플이 함께 표시됐기 때문입니다.
- **어떻게(How)**: 선택 배경을 즉시 갱신하는 일반 컨테이너로 바꾸고 필터의 스플래시·하이라이트 효과를 비활성화했습니다.
- **검증(Verification)**: Dart 포맷과 `git diff --check`를 확인했습니다. 단순 시각 효과 수정이며 사용자 요청에 따라 별도 테스트는 실행하지 않았습니다.
- **추적(Tracking)**: 브랜치 `fix/content-filter-flicker`, 코드 커밋 `bd4a152`, PR·배포 전
- **상태(Status)**: 로컬 수정·코드 커밋 완료
- **위험 및 후속 작업(Risks/Follow-up)**: 운영 반영 후 실제 PWA에서 선택 상태가 즉시 전환되는지 확인합니다.
